...
const DrawerUi = props => {
  ...
  return /*#__PURE__*/React.createElement(_core.Drawer, {
    ...
    h: `calc(100vh - ${(0, _core.rem)(80)})`,
    mx: "-md",
    className: "relative"
  }, /*#__PURE__*/React.createElement(_core.Divider, {
    my: "sm"
  }), HeadData?.map((el, index) => /*#__PURE__*/React.createElement(_core.Box, {
    key: index,
    className: "w-full"
    ...
    ml: 10,
    mx: "xl"
  }, (SubHeadData[index]?.selectData[0]?.items || []).map(el => /*#__PURE__*/React.createElement(_core.Text, {
    className: `!pl-[3rem] cursor-pointer hover:bg-[#F1F3F5]  ${SubHeadData[index]?.id == el.id && '!font-bold'}`,
    key: Math.random(),
    py: "sm",
    size: "md"
  }, el.name))))), SubHeadData[index]?.items?.map(el => /*#__PURE__*/React.createElement(_core.Text, {
    px: "xl",
    py: "sm",
    size: "md",
    fw: 500,
